...
def snv(input_data):
    """ 
    Perform standart normal variate transformation
    """
...
def preprocess_data(data,Smooth, Standartization, Baselin_corr, median_filter_parameter,first_diff, cut_range_from):
    """
    Data preprocessing
    """
    all_data = []
    for  k in [data]:
        ...
        if median_filter_parameter:
            ...
            new_X_train  = np.zeros((df.shape[0],df.shape[1]//median_filter_parameter))   
...
def add_noise(X,y):
    """
    Augments data twice a size with a noise samples
    X - 
    y - 
    """
    
    noise = np.random.uniform(low=0.1, high=0.7,size =X.shape) * np.random.normal(0,np.mean(np.std(X)), X.shape)
    new_signal = X + noise
    X = pd.DataFrame(X)
...
def classification_rep(y_test,y_pred):
    ...
    temp['lde'] = y_pred
    print(classification_report(temp.classes, temp.lde ,target_names= target_names))
...
def plot_2d_lda_space(model, X, y):
    ...
    fig.update_layout( 
        paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)',  width=1200,
        height=900,legend=dict(
        yanchor="top",
        y=0.99,
        xanchor="left",
        x=0.01
...
def train_model(X_train,y_train, model, show_cv_score = True):
    ...
    if show_cv_score:
        print("Model trained with accuracy on CV: %0.4f (+/- %0.4f)" % (scores.mean(), scores.std() * 2))
...
def get_accuracy_vs_num_features(data, weights,x_axis):
    ...
    y1, y2, y3 = weights
    features, plot_helper1, plot_helper2 = select_features_for_model(y1,y2,y3,x_axis)
    accs = []
    sub_plots_imp = []
    sub_plots = []
    X_train1, X_test1, y_train1, y_test = train_test_split(np.array(data)[:,:-2],
                                                            np.array(data)[:,-2:], test_size = 0.35)
    for feature in range(3,len(features)):
        ...
        if feature<len(plot_helper2) and feature>len(plot_helper1) & feature % 25 ==0:
...
#         train_model(lda,X_train,y_train)
...
def select_features_for_model(y1, y2, y3, x_axis):
    ...
    """
    cond2 = ((x_axis>433) & (x_axis<460)) | ((x_axis>868) & (x_axis<883))
    cond3 = ((x_axis>944) & (x_axis<958)) | ((x_axis>1235) & (x_axis<1252))
    cond1 = ((x_axis>532) & (x_axis<545)) | ((x_axis>1077) & (x_axis<1092))
    ...
    arrr1 = np.array(y2)[((np.where(cond2))[0])[np.argsort(np.array(y2)[((np.where(cond2) )[0])])][::-1]]
...
def plot_accs(x_axis,data,weights):
    ...
    """
    accs, sub_plots, features = get_accuracy_vs_num_features(data, weights,x_axis)
    levels = [0,0.7,1.4]
    ...
    fig = make_subplots(
        ...
        specs=[[{}],
               [{}] ,
               [{}]],
        subplot_titles=("(a) First 25 selected features","(b) First 50 selected features", "(c) Result"))

    [fig.add_trace(go.Scatter(x = np.array(x_axis),y =levels[i]+ np.mean(np.array(data[data.classes==i])[:,:-2], axis = 0),
                              marker = dict(color = colorss[i]), name = target_names[i]),
                     row=1, col=1) for i in range(3)]

    for j in sub_plots[0]:
            fig.add_vline(x=j, line_dash="solid", row=1, col=1, line_color = 'green', opacity=0.15,)
    [fig.add_trace(go.Scatter(x = np.array(x_axis),
                              y = levels[i]+np.mean(np.array(data[data.classes==i])[:,:-2], axis = 0), 
                              marker = dict(color = colorss[i]),showlegend=False),
                     row=2, col=1) for i in range(3)]


    for j in sub_plots[1]:
            fig.add_vline(x=j, line_dash="solid", row=2, col=1, line_color = 'green', opacity=0.15,)
    fig.add_trace(go.Scatter(x=np.arange(3,len(features)), y=accs, marker = dict(color = 'cyan'),showlegend=False),
                     row=3, col=1)
    ylevl = 2
    ...
    fig.add_annotation(x=1084, y=ylevl,
                text="<b>VI<b>",
                showarrow=False,row=2, col=1)
    fig.update_xaxes(row=1, col=1,showline=True, linewidth=1, linecolor='black', mirror=True)
    fig.update_xaxes(row=2, col=1,showline=True, linewidth=1, linecolor='black', mirror=True)

    fig.update_yaxes( row=1, col=1,showline=True, linewidth=1, linecolor='black', mirror=True,
                    showticklabels=False)
    fig.update_yaxes( row=2, col=1,showline=True, linewidth=1, linecolor='black', mirror=True
                    ,showticklabels=False)
    fig.update_yaxes(title_text="Accuracy", row=3, col=1,showline=True, linewidth=1, linecolor='black', mirror=True)
    fig.update_xaxes(title_text="Number of features used (bands)", row=3, col=1,showline=True, linewidth=1, linecolor='black', mirror=True)
    fig.show()
    
def save_weights(y1,y2,y3):
    """
    This function saving weights obtained from previous step
    """
    np.save('y1',y1)
    np.save('y2',y2)
    np.save('y3',y3)   
...
def plot_weights(y1,y2,y3,x_axis,data):
    ...
    all_levels = [0,0.5,1]
    barnames = ['Lung ACE weights','Seminal fluid ACE weights','Heart ACE weights']   
    target_names =  ['Lung ACE','Seminal fluid ACE','Heart ACE']    
    ...
    for i,y in enumerate([y1,y2,y3]):
        ...
        fig.add_trace(go.Scatter(x = x_axis, y= all_levels[i] + np.mean(np.array(data[data.classes==i])[:,:-2], axis = 0)
    ...
    fig.update_layout(width=2000,
        ...
        paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
    fig.update_yaxes(title = '<b>I<b>',title_font=dict(size = 40),tick0=0,dtick=1,showline=True, linewidth=1, linecolor='black', mirror=True)
    fig.update_xaxes(title = '<b> Raman shift cm-1 <b>',title_font=dict(size = 40),dtick=100,showline=True, linewidth=1, linecolor='black', mirror=True)
    fig.show()
...
def plot_intervals(x_axis, weights, data):
    ...
    y1,y2,y3 = weights
    classes = [1,0,2]
    ranges = [[430,463], [865,885],[1075,1094], [530,547],  [941,965], [1231,1258]]
    yranges = [[-0.06,0.6],[-0.06,0.6],[-0.06,0.6],[-0.06,0.6],[-0.06,0.8],[-0.06,1]]
    barnames =  ['<b>Seminal fluid ACE            <b>','<b>Lung ACE              <b>','<b>Heart ACE         <b>']    
    link_names = ['Sceletal def.', '','', 'COO‾ def',   'C-C str.', 'CH₂ wag.' ]
    fig = make_subplots(
        rows=2, cols=3,
        subplot_titles=("<b>Interval I<b>","<b>Interval III<b>", "<b>Interval V<b>", '<b>Interval II<b>', '<b>Interval IV<b>', 
                        '<b>Interval VI<b>'))
    for i,y in enumerate([y2,y1,y3]):
        for j in range(2):
            fig.add_trace(go.Bar(x = x_axis, y= y,width=2.5 , legendgroup=str(i),  
                        legendgrouptitle_text=barnames[i] ,name = link_names[2*i+j]),
                     row=j+1, col=i+1) 


            klss = np.array(data[data.classes==classes[i]])[:,:-2]
            fig.add_trace(go.Scatter(x = np.array(x_axis),y = np.mean(klss, axis = 0), marker = dict(color = colorss[i]),showlegend=False),
                     row=j+1, col=i+1)
            fig.add_trace(
                  go.Scatter(x =  np.array(x_axis),

                      y= np.mean(klss, axis = 0)+2*np.std(klss.astype(float), axis = 0),
                      mode='lines',
                      line=dict(width=0),
                          showlegend=False
                  ),row=j+1, col=i+1)
            ...
            fig.update_xaxes(tickfont=dict(size=14), tickangle=45,range = ranges[2*i+j],dtick=5,
                     row=j+1, col=i+1,showline=True, linewidth=1, linecolor='black', mirror=True)
            fig.update_yaxes(tickfont=dict(size=14),range = yranges[2*i+j],
                     row=j+1, col=i+1,showline=True, linewidth=1, linecolor='black', mirror=True)

    fig.update_layout(width=1000,
                height=750,legend=dict(
        orientation="h",
